...
{
    public static final List<Integer> SCOPES = Arrays.asList(ENGINE_SCOPE,
                                                             GLOBAL_SCOPE);
    ...
    public GleamScriptContext(Interpreter interpreter)
    {
        ...
        executionContext =
                new ExecutionContext(interpreter,
        ...
        Interpreter.setGlobalEnv(interpreter,
                                 new Environment(executionContext));
    }

    @Override
    public void setBindings(Bindings bindings, int scope)
    {
        GleamBindings gleamBindings = toGleamBindings(bindings);
        super.setBindings(bindings, scope);
        switch (scope) {
            case GLOBAL_SCOPE:
                globalScope = gleamBindings;
                if (gleamBindings != null) {
                    Interpreter.setGlobalEnv(executionContext.getInterpreter(),
                                             gleamBindings);
                }
                else {
                    Interpreter.setGlobalEnv(executionContext.getInterpreter(),
    ...
    private GleamBindings toGleamBindings(Bindings bindings)
    {
        if (bindings == null) {
            return null;
        }
